 ...
 * good or bad for THIS metric. `improve` says which direction is good ('down' for
 * open cases, null for neutral metrics like alerts sent) so colour never lies.
 */
function TrendChip({ delta, improve, label }) {
  ...
  return (
    <Stack
      ...
      spacing={0.25}
      component="span"
      aria-label={`${delta > 0 ? 'up' : delta < 0 ? 'down' : 'no change'} ${Math.abs(delta)} ${label}`}
...
/**
 * KPI tile in the clean analytics style: muted label with a small icon, a large
 * value, and an inline trend chip with a "vs last week" caption.
 */
