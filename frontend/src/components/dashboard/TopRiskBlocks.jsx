...
// Blue heat scale: darkest for the highest volume, fading out as counts drop, so
...
const BREACH_INK = '#B3261E';
const HEAT = ['#1E3A5F', '#2C5687', '#4A7CB0', '#7C9DBF', '#A9BDD1'];
          ...
          <>
            <Stack spacing={1.75}>
              {shown.map((b, i) => {
                ...
                const ink = isTop && over ? BREACH_INK : HEAT[Math.min(i, HEAT.length - 1)];
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 2.5, flexWrap: 'wrap', rowGap: 0.75, alignItems: 'center' }}>
              {[
                [BREACH_INK, 'Worst block - breached'],
                [HEAT[0], 'Higher volume'],
                [HEAT[HEAT.length - 1], 'Lower volume'],
              ].map(([c, l]) => (
                <Stack key={l} direction="row" spacing={0.6} sx={{ alignItems: 'center' }}>
                  <Box aria-hidden sx={{ width: 10, height: 10, borderRadius: '2px', bgcolor: c, flexShrink: 0 }} />
                  <Typography sx={{ fontSize: 12, color: BRAND.text }}>{l}</Typography>
                </Stack>
              ))}
            </Stack>
            {hidden > 0 && (
